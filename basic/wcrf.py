...
class WeightedCRF(nn.Module):
    ...
    def _calculate_PZ(self, feats, mask,weights):
        """
            input:
        ...
        weighted_score = final_partition * weights
        return weighted_score.sum(), scores
    ...
    def _score_sentence(self, scores, mask, tags,weights):
        """
            input:
        ...
        weighted_end_energy = end_energy * weights.view(-1,1)
        ...
        #set instance weight
        # energy calc not zero value 注意 tg 维度 batch size在第二位
        weighted_tg_energy = tg_energy * weights.view(-1,1).repeat(1,seq_len).transpose(1,0)
        tg_energy = weighted_tg_energy.masked_select(mask.transpose(1,0))
        ...
        gold_score = tg_energy.sum() + weighted_end_energy.sum()
        #gold_score = tg_energy.sum() + end_energy.sum()
        return gold_score

    def neg_log_likelihood_loss(self, feats, mask, tags,weights):
        # nonegative log likelihood
        batch_size = feats.size(0)
        forward_score, scores = self._calculate_PZ(feats, mask,weights)
        gold_score = self._score_sentence(scores, mask, tags,weights)
