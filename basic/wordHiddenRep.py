...
class WordHiddenRep(nn.Module):
    def __init__(self, args):
        ...
        self.input_size = args.embedding_dim# + data.char_hidden_dim
        if self.use_char:
    ...
    # word hidden rep
        if args.encoderExtractor == "LSTM":
            self.lstm = nn.LSTM(self.input_size, self.hidden_dim, num_layers=1, batch_first=True, bidirectional=True)
        else:
